...
<xsl:template match="om:Presence">
   <xsl:copy>
      ...
      <xsl:variable name="p"><xsl:value-of  disable-output-escaping="yes" select="$points"/></xsl:variable>
...
<xsl:template match="om:TestParameters">
   <xsl:copy>
      <xsl:apply-templates select="om:Sampler"/>
      ...
      <xsl:apply-templates select="om:Statistics"/>
    </xsl:copy>
</xsl:template>
