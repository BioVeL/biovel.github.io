...
<!--
    <xsl:for-each select='str:tokenize($presence_points)'>
      <xsl:element name="Point" namespace="http://openmodeller.cria.org.br/xml/1.0">
...
<xsl:template match="om:ModelParameters">
   <xsl:copy>
      ...
      <xsl:element name="Options" namespace="http://openmodeller.cria.org.br/xml/1.0">
          <xsl:choose>
            <xsl:when test="$spatially_unique = 'yes' or $environmentally_unique = 'yes'">
              <xsl:element name="OccurrencesFilter" namespace="http://openmodeller.cria.org.br/xml/1.0">
                <xsl:choose>
                  <xsl:when test="$environmentally_unique = 'yes'">
                    <xsl:element name="EnvironmentallyUnique" namespace="http://openmodeller.cria.org.br/xml/1.0"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <xsl:element name="SpatiallyUnique" namespace="http://openmodeller.cria.org.br/xml/1.0"/>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:element>
            </xsl:when>
